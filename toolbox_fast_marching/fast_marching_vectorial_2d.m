function [D,S,Q] = fast_marching_vectorial_2d(W, start_points, options, fx,fy,N)
...
if exist('perform_vectorial_propagation_2d','file')~=0
    [D,S,Q] = perform_vectorial_propagation_2d(W,start_points-1,end_points-1,nb_iter_max, H, L, values,  fx, fy, N);
    Q = Q+1;
else
    if nargin<4
        [D,S] = perform_vectorial_propagation_2d_slow(W,start_points,end_points,nb_iter_max);
    else
        [D,S] = perform_vectorial_propagation_2d_slow(W,start_points,end_points,nb_iter_max, H);
    end
end
