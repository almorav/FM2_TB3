function [D,S] = perform_fast_marching_2d(W, start_points, options, H)

% perform_fast_marching_2d - launch the Fast Marching algorithm.
%
%   [D,S] = perform_fast_marching_2d(W, start_points, options, H)
%
...
if exist('perform_front_propagation_2d')~=0
    if nargin<4 
        ...
        [D,S] = perform_front_propagation_2d(W,start_points-1,end_points-1,nb_iter_max, H);
    end
else
    if nargin<4
