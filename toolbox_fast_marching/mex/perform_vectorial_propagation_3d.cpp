...
#define FX_(i,j,k) ACCESS_ARRAY(FX,i,j,k)
#define FY_(i,j,k) ACCESS_ARRAY(FY,i,j,k)
#define FZ_(i,j,k) ACCESS_ARRAY(FZ,i,j,k)
#define N_(i,j,k) ACCESS_ARRAY(N,i,j,k)
	...
	while( !fh_isempty(open_heap) && num_iter<nb_iter_max && !stop_iteration )
	{
		...
		for( int s=0; s<6; ++s )
		{
			...
			if( ii>=0 && jj>=0 && ii<n && jj<p && kk>=0 && kk<q && bInsert )
			{
				...
				if( ii<n-1 )//D_(ii+1,jj,kk) es el menor
					{a1 = D_(ii+1,jj,kk);  
                     double aa=0.0;
                     aa=(alpha/N_(ii,jj,kk)) * FX_(ii,jj,kk);
                     }
                
				if( ii>0 )//D_(ii-1,jj,kk) es el menor
				...
				if( jj<p-1 )
					{a2 = D_(ii,jj+1,kk);
                     double bb=(alpha/N_(ii,jj,kk)) * FY_(ii,jj,kk);
                     }
				if( jj>0 )
				...
				if( kk<q-1 )
					{a3 = D_(ii,jj,kk+1);
                    double cc=(alpha/N_(ii,jj,kk)) * FZ_(ii,jj,kk);
                    }
				if( kk>0 )
