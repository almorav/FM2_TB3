...
double* FX;
double* FY;
double* N; // this is the peytres Q
		...
		*/
		...
		for( int k=0; k<4; ++k )
		{
			...
			if( ii>=0 && jj>=0 && ii<n && jj<p && bInsert )
			{
                ...
                // notation of "Clement Petres"
                // UA = a1 = U(A1)
                // UB = a2 = U(A2)
                // Tk = P
                // Uk = A1
                double a = (alpha/N_(ii,jj)) * FX_(ii,jj);
                double b = (alpha/N_(ii,jj)) * FY_(ii,jj);
                double lambda = P + alpha;
                double Lv = lambda / (1-(a*a+b*b));
                double U = 2 - (a+b)*(a+b);
                double temp = (U*Lv*Lv - ((a2-Lv*b)-(a1-Lv*a)) * ((a2-Lv*b)-(a1-Lv*a)));
                double delta = ((4*lambda)/ Lv)*temp;
                double V = 2*a1*(a*a+a*b-1) +2*a2*(b*b+a*b-1) +2*lambda*(a+b);
                double A1 = 0;
                
                if(a1<a2)
                {
                    if (((Lv*a2-Lv*b)-(a1-Lv*a))*((Lv*a2-Lv*b)-(a1-Lv*a))<Lv*Lv*U){
                        // if( P*P > (a2-a1)*(a2-a1) ){
                        //  double delta = 2*P*P-(a2-a1)*(a2-a1);
                        //   A1 = (a1+a2+sqrt(delta))/2.0;
                        A1 = (-V+sqrt(delta))/(2*U);
                         //A1=a1+(P+alpha)/ (1+alpha/N_(ii,jj) * fmaxf(FX_(ii,jj),FY_(ii,jj)));
